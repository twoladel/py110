...
VOWELS = {'a', 'e', 'i', 'o', 'u'}
list_of_vowels = [char for list in dict1.values()
                        for word in list
...
'''
input: dictionary with lists of strings as values
output: list of vowels found in strings
explicit: duplicate vowels stored is ok
implicit:
    each string is one word
    lists have varying amounts of words

A- 
access each value (list) in the dict
for each list
